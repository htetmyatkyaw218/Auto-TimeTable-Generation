                         ...
                         <div class="form-group" style="position: relative;">
                            <label for="defaultSelect">Department Code <span style="color: red; font-size: 1.2em;">*</span></label>
                              <div class="custom-select-box" id="customSelect">
                                <span id="selectedText">Select Department Code</span>
                                <span class="remove-last" id="removeLastBtn" title="Remove last" style="display: none; ">×</span>
                              </div>
                          ...
                          <input type="hidden" name="dept_code" id="selectedDepartmentsInput" required>
                        </div>
...
dropdown.addEventListener('click', function(event) {
  ...
  function updateDisplay() {
    if (selectedValues.length === 0) {
      ...
      removeLastBtn.style.display = "inline";
    }
...
<script>
    ...
    var status = "<?php echo $_GET['status']; ?>"; // PHP to JS
    var content = {};
    ...
    if (status === 'error') {
        ...
        content.message = 'Error: Department Code is already exist! ,Click to Check ';
        content.title = "Warning";
        content.icon = "fa fa-exclamation-triangle";
        var state = "warning"; // yellow color

        // Setting URL and target for redirection
        content.url = "teacher.php";
        content.target = "_blank"; // Opening in a new tab
        ...
        $.notify(content, {
            ...
            time: 1000,
            delay: 7000, // Show the notification for 5 seconds
            animate: {
        ...
        // Clean up the URL (remove 'status' from the query string)
        if (window.history.replaceState) {
            const url = new URL(window.location);
            url.searchParams.delete('status');
            window.history.replaceState({}, document.title, url.pathname + url.search);
