...
if ($_POST['action'] == 'delete') {
    ...
    $stmt = $con->prepare("DELETE FROM teaches WHERE year_id = ? AND section_id = ?");
...
if (isset($_POST['update'])) {
    $teaches_id = mysqli_real_escape_string($con, $_POST['teaches_id']);
    $teacher_names = $_POST['tname']; // this is an array
    $teacher_name_string = implode(',', $teacher_names); // e.g. "John Doe, Jane Smith"


    $query = "UPDATE teaches SET teacher_name='$teacher_name_string' WHERE teaches_id='$teaches_id'";
    $query_run = mysqli_query($con, $query);

    if ($query_run) {
        header("Location: Assign_Teacher.php?status=up_success");
        exit(0);
    } else {
        header("Location: Assign_Teacher.php?status=error");
        exit(0);
    }
}
